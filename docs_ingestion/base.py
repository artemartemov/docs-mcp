...
class DocumentContent:
    ...
    def is_valid(self) -> bool:
        ...
        return (
            len(self.content.strip()) >= 50 and  # Minimum content length
            self.metadata.framework and
            self.metadata.source and
            self.metadata.title and
            self.metadata.url
        )
...
class DocumentationIngester:
    ...
    async def ingest_from_source(self, source: BaseDocumentationSource) -> IngestionStats:
        ...
        for batch_idx in range(total_batches):
            ...
            await self._process_batch(source, batch_identifiers, batch_idx + 1, total_batches)
    ...
    async def _process_batch(
        ...
        logger.info(f"[Batch {batch_num}/{total_batches}] Processing {len(identifiers)} items...")
