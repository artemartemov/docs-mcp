...
async def main():
    ...
    logger.info("🚀 Starting comprehensive Figma API documentation extraction from JSON")
    ...
    try:
        ...
        figma_source = FigmaJsonDocsSource(json_file_path=json_file_path, version="latest")
    ...
    try:
        ...
        if collection:
            ...
            test_results = collection.query(
                query_texts=["How to authenticate with Figma API"],
                n_results=3
            )
            
            if test_results.get('documents'):
                logger.info(f"✅ Search test successful - found {len(test_results['documents'][0])} relevant documents")
