...
async def ingest_python_docs(
    version: str = "3", test_mode: bool = False
) -> Dict[str, Any]:
...
async def ingest_generic_docs(
    source_class, source_name: str, version: str = "latest", test_mode: bool = False
) -> Dict[str, Any]:
...
async def ingest_multiple_sources(
    sources: List[str], test_mode: bool = False
) -> Dict[str, Any]:
    ...
    for source_name in sources:
        try:
            if source_name == "python":
                ...
                result = await ingest_generic_docs(
                    FastAPIDocsSource, source_name, version, test_mode
                )
                results[source_name] = result
                ...
                version = config.get("default_version", "latest")
                result = await ingest_generic_docs(
                    ReactDocsSource, source_name, version, test_mode
                )
                results[source_name] = result
                ...
                version = config.get("default_version", "latest")
                result = await ingest_generic_docs(
                    SwiftUIDocsSource, source_name, version, test_mode
                )
                results[source_name] = result
                ...
                version = config.get("default_version", "latest")
                result = await ingest_generic_docs(
                    TailwindDocsSource, source_name, version, test_mode
                )
                results[source_name] = result
                ...
                version = config.get("default_version", "latest")
                result = await ingest_generic_docs(
                    FigmaScreenshotDocsSource, source_name, version, test_mode
                )
                results[source_name] = result
            else:
    ...
    for source_name, result in results.items():
        if "collection_stats" in result:
            ...
            if "total_documents" in stats:
                logger.info(
                    f"📚 ChromaDB Collection: {stats['total_documents']} total documents"
                )
...
async def handle_retry_ingestion(
    retry_file: str, source_name: str, test_mode: bool, output_file: str = None
):
...
"""
    ...
    if retry_stats.failed_ingestions == 0:
        ...
        logger.info(
            f"⚠️  {retry_stats.failed_ingestions} URLs still failing after retry"
        )
...
async def check_prerequisites():
    ...
    try:
        ...
        logger.info(
            f"✅ ChromaDB connected: {stats.get('total_documents', 0)} existing documents"
        )
    except Exception as e:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("https://docs.python.org/3/objects.inv") as response:
                if response.status == 200:
                    ...
                    logger.warning(
                        f"⚠️  Unexpected response from Python docs: {response.status}"
                    )
    except Exception as e:
...
Examples:
  ...
  python ingest_documentation.py --source all
    ...
    parser.add_argument(
        "--retry", help="Path to failed URLs JSON file for retry ingestion"
    )
    ...
    try:
        # Handle retry mode
        if args.retry:
            await handle_retry_ingestion(
                args.retry, args.source, args.test, args.output
            )
            return
