...
class ReactDocsSource(BaseDocumentationSource):
    ...
    def __init__(self, version: str = "latest"):
        ...
        self.priority_sections = {"learn", "reference", "community", "blog"}
    ...
    async def _discover_from_sections(self) -> Set[str]:
                ...
                async with self.session.get(section_url) as response:
                    if response.status == 200:
                        ...
                        for link in all_links:
                            ...
                            if full_url.startswith(self.base_url) and any(
                                f"/{sec}/" in full_url or full_url.endswith(f"/{sec}")
        ...
        logger.info(
            f"📂 Found {len(section_urls)} URLs from recursive section discovery"
        )
        return section_urls
    ...
    def _should_include_url(self, url: str) -> bool:
        ...
        if any(
            section in path for section in ["learn", "reference", "blog", "community"]
        ):
            return True
