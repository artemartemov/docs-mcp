...
class FigmaJsonDocsSource(BaseDocumentationSource):
        ...
        try:
            ...
            for section in self.sections:
                ...
                if section_id and word_count >= 25:  # Minimum word count threshold
                    ...
                    if section_id not in unique_sections or word_count > unique_sections[section_id]['word_count']:
    ...
    def _categorize_section(self, section_id: str) -> str:
        ...
        if any(term in section_id_lower for term in ['intro', 'authentication', 'auth']):
            ...
            return "users_teams"
        elif any(term in section_id_lower for term in ['component', 'style', 'library']):
        ...
        elif any(term in section_id_lower for term in ['payment', 'scim', 'discovery', 'analytics']):
            return "enterprise_features"
        elif any(term in section_id_lower for term in ['type', 'prop']):
            return "type_definitions"
        elif any(term in section_id_lower for term in ['endpoint', 'get-', 'post-', 'put-', 'delete-']):
    ...
    async def extract_content(self, section_id: str) -> Optional[DocumentContent]:
        ...
        try:
            ...
            title = section_data.get('title', section_id.replace('-', ' ').replace('_', ' ').title())
    ...
    def _generate_tags(self, section_id: str, title: str, content: str) -> List[str]:
        ...
        if any(term in content_lower for term in ["endpoint", "api", "request", "response"]):
        ...
        elif any(term in title.lower() for term in ["advanced", "webhook", "scim", "enterprise"]):
