...
class FigmaDocsSource(SPADocumentationSource):
    ...
    async def discover_content(self) -> List[str]:
        ...
        logger.info(
            "💡 For comprehensive API docs, consider using Figma's REST API directly"
        )
            ...
            for selector in figma_content_selectors:
                try:
                    content = await asyncio.wait_for(
                        page.text_content(selector), timeout=3.0
                    )
    ...
    async def postprocess_metadata(
        ...
        if any(
            platform in url_lower or platform in title_lower
        ...
        if any(
            ...
            for term in ["oauth", "token", "security", "permission", "auth"]
        ):
