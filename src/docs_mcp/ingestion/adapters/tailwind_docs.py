...
class TailwindDocsSource(BaseDocumentationSource):
    ...
    def _extract_section_from_url(self, url: str) -> str:
        ...
        if len(path_parts) >= 2 and path_parts[0] == "docs":
            ...
            if section in [
                ...
                return "spacing"
            elif section.startswith("font-") or section in [
                "text-align",
                "text-color",
                "text-decoration",
                "letter-spacing",
                ...
                return "typography"
            elif section.startswith("bg-") or section in [
                "background-color",
                "background-image",
                "background-size",
            ]:
                ...
                section in ["flex", "grid", "justify-content", "align-items"]
                or section.startswith("flex-")
                or section.startswith("grid-")
            ):
                return "flexbox_grid"
            else:
                return section
    ...
    def _determine_doc_type(self, url: str, title: str, content: str) -> str:
        ...
        if "installation" in url_lower or "setup" in title_lower:
            ...
            concept in url_lower
            for concept in ["utility-first", "responsive", "dark-mode", "hover-focus"]
        ):
            return "core_concept"
        elif any(
            term in url_lower for term in ["customization", "configuration", "plugins"]
        ):
    ...
    def _generate_tags(self, url: str, title: str, content: str) -> List[str]:
        ...
        if any(
            term in title.lower()
            for term in ["installation", "getting started", "basics"]
        ):
