...
class FigmaFileDocsSource(BaseDocumentationSource):
    ...
    def _extract_sections(self) -> List[str]:
        ...
        for element in section_elements:
            ...
            if section_id and (
                section_id in self.section_mapping
                or any(
                    known_id in section_id for known_id in self.section_mapping.keys()
                )
            ):
        ...
        doc_sections = self.soup.find_all(
            "div", class_=re.compile(r"developer_docs--section")
        )
        for element in doc_sections:
            section_id = element.get("id")
            if section_id and section_id not in self.sections:
                # Check if this is a known section
                if section_id in self.section_mapping or any(
                    known in section_id for known in self.section_mapping.keys()
                ):
            ...
            for header in headers:
                ...
                if section_id and section_id not in self.sections:
                    ...
                    if parent:
                        ...
                        sections.append(section_id)
                        logger.debug(
                            f"Found section by header text: {header_text} -> {section_id}"
                        )
    ...
    def _extract_section_title(self, section_element: Tag, section_id: str) -> str:
        ...
        for tag in ["h1", "h2", "h3"]:
            header = section_element.find(
                tag, class_=re.compile(r"sectionHeader|subsectionHeader")
            )
            if header:
    ...
    def _extract_clean_content(self, section_element: Tag) -> str:
        ...
        for word in words:
            ...
            if (
                len(word) > 1
                and not any(pattern in word for pattern in self.skip_patterns)
                and not word.startswith("http")  # Skip URLs for now
                and word
                not in {"Get", "What's", "this?", "My", "apps", "Create", "new", "app"}
            ):
    ...
    def _determine_doc_type(self, section_id: str, title: str, content: str) -> str:
        ...
        elif "endpoint" in content_lower or any(
