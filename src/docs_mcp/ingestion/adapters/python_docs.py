...
class PythonDocsSource(BaseDocumentationSource):
    ...
    async def discover_content(self) -> List[str]:
        ...
        try:
            ...
            logger.info(
                f"📚 Loaded inventory with {len(self.inventory.objects)} objects"
            )
            ...
            for obj in self.inventory.objects:
                ...
                if (
                    obj.role in ["data", "attribute", "exception"]
                    and obj.domain == "py"
                ):
                    continue
    ...
    def _should_include_url(self, url: str) -> bool:
        ...
        url_lower = url.lower()
        has_priority_section = any(
            section in url_lower for section in self.priority_sections
        )
