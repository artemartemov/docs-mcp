...
try:
    ...
    logger.warning(
        "Playwright not available. SPA adapters will have limited functionality."
    )
...
class SPADocumentationSource(BaseDocumentationSource, ABC):
    ...
    def __init__(self, name: str, base_url: str, use_browser: bool = True):
        ...
        if not self.use_browser:
            logger.warning(
                f"🌐 {name}: Browser automation disabled - using fallback HTML parsing only"
            )
        else:
            logger.info(
                f"🚀 {name}: Browser automation enabled for complete SPA support"
            )
    ...
    async def extract_content_with_browser(self, url: str) -> Optional[DocumentContent]:
        ...
        if not self.use_browser or not self.browser:
            logger.warning(
                f"Browser not available for {url}, falling back to HTML parsing"
            )
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    f"⚠️  SPA content loading timeout for {url}, proceeding with available content"
                )
            ...
            try:
                content_text = await asyncio.wait_for(
                    self._extract_main_content(page, url), timeout=8.0
                )
            except asyncio.TimeoutError:
            ...
            if not content_text or len(content_text.strip()) < 100:
                logger.warning(
                    f"⚠️  Insufficient content extracted from {url} ({len(content_text) if content_text else 0} chars)"
                )
                return await self.extract_content_fallback(url)
            ...
            logger.info(
                f"✅ Successfully extracted {len(content_text)} chars from {url}"
            )
